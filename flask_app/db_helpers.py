...
def merge_duplicate_work_night():
    """Merge duplicate 'Work night' categories into 'Work Night'

    Finds any case variations of "work night" and merges them into
    the properly capitalized "Work Night", preserving all time logs.
    ...
    try:
        ...
        if len(categories) == 0:
            return False, "No 'Work Night' categories found"
        elif len(categories) == 1:
            return True, f"Only one 'Work Night' category exists - no duplicates to merge"
        ...
        if not merge_categories:
            return True, "No duplicate categories to merge"
        ...
        return True, f"Successfully merged {len(merge_categories)} duplicate(s) and moved {total_logs_moved} time log(s) to 'Work Night'"
