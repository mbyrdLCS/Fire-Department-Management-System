...
For use on PythonAnywhere

IMPORTANT: This will clear existing firefighter and time log data!
Vehicles and checklists will NOT be affected.
"""
import json
import sqlite3
import sys
...
# Paths for PythonAnywhere
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
OLD_APP_DATA_DIR = os.path.join(SCRIPT_DIR, "migration_data")
NEW_APP_DB = os.path.join(SCRIPT_DIR, "flask_app/database/fire_dept.db")
...
def load_old_app_data():
    ...
    user_data_path = os.path.join(OLD_APP_DATA_DIR, "user_data.json")
    if not os.path.exists(user_data_path):
        print(f"❌ Error: {user_data_path} not found!")
        print(f"   Make sure migration_data folder is in {SCRIPT_DIR}")
        sys.exit(1)
    ...
    # Load categories
    categories_path = os.path.join(OLD_APP_DATA_DIR, "categories.json")
...
def verify_migration(conn):
    ...
    # Count vehicles (should be unchanged)
    ...
    print(f"  Vehicles: {vehicle_count} (preserved)")
    print(f"\n  Top 5 by Hours:")
...
def main():
    ...
    if '--confirm' not in sys.argv:
        ...
        print("  python3 migrate_for_pythonanywhere.py --confirm")
        return
